      ...
      <div className="backe">
        {admin ? (
          <nav className="navbar navbar-expand-lg navbar-light bg-light container d-flex navs">
            <ul className="navbar-nav container d-flex justify-content-between">
              <img id="logo_elrus" src={`/elbruss.svg`} />
                  ...
                  activeclass.map((el) => {
                    return (
                      <li className="nav-item">
                        <Link
                          onClick={() => classHandler(el.name)}
                          to={el.url}
                          className={`nav-link  ${el.current ? 'active' : ''}`}
                        >
                          {el.name}
                        </Link>
                      </li>
        ...
        {!admin ? (
          <nav className="navbar navbar-expand-lg navbar-light bg-light container navs">
            <ul className="navbar-nav container d-flex justify-content-between">
              {isAuth && <img id="logo_elrus" src={`/elbruss.svg`} />}
              <div className="navbar-nav container d-flex justify-content-between nav_div">
                {isAuth &&
                  activeclassUser.map((el) => {
                    return (
                      <li className="nav-item">
                        <Link
                          onClick={() => classHandlerUser(el.name)}
                          to={
                            el.name === 'Профиль'
                              ? el.url + student._id
                              : el.url
        ...
        {!admin && !isAuth ? (
          <nav className="navbar navbar-expand-lg navbar-light bg-light container navs">
            <div className="container d-flex justify-content-center">
              {!isAuth &&
                activeclassLogin.map((login) => {
                  return (
                    <li className="nav-item login_reg">
                      <Link
                        onClick={() => classHandlerLogin(login.name)}
                        to={login.url}
                        className={`nav-link  ${login.current ? 'active' : ''}`}
                      >
                        {login.name}{' '}
                      </Link>
                    </li>
