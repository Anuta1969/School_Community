import {
  ...
  updateUserProfileAC,
  addResumeUserAC,
  updateUserFromAllState,
  addPhotoUserFromAllAC,
  addResumeUserFromAllAC,
} from '../actionCreators/actionCreatorStudent';

export const ThunkAddResumeUser = (id, dats) => {
  return async (dispatch) => {
    const response = await fetch(`/student/addresume/${id}`, {
      method: 'POST',
      body: dats,
    });
...
export const ThunkAddPhotoUser = (id, dats) => {
  return async (dispatch) => {
    const response = await fetch(`/student/addphoto/${id}`, {
      method: 'POST',
      body: dats,
    });
    const result = await response.json();
